import CDTheme from '@component-driven/mdx-deck-theme';

export const theme = { ...CDTheme };

<Primary>

# Resilient ⚛ Component Libraries

</Primary>

---

<Secondary>

# [git](https://github.com/component-driven/resilient-component-libraries)

</Secondary>

---

## Remote workshop

1. Please keep your camera on for a more personal feeling.
1. "Rise hand". Ask questions any time!
1. Exercises with teams 5-6 people in breakout rooms.
1. Use "Ask a question" in the breakout room to get attention
1. Someone should always share their screen.

---

## Plan of the workshop

1. Introductions
1. Why developing a component library
1. Primitives
1. UI Composition
1. Behavior Composition
1. UI Patterns
1. Discussion and Q&A

---

## Andrey Okonetchnikov

UI developer with experience in both design and development who specializes in interaction design, design systems and modern frontend development. Currently leading a design system team @ Adverity.

[@okonetchnikov](https://twitter.com/okonetchnikov)

---

## Artem Sapegin

UI engineer working with web technologies since 1999. Creator of React Styleguidist and component-driven development 🥑

[@iamsapegin](https://twitter.com/iamsapegin)

---

<Secondary>

## What's your name and what's on your mind?

</Secondary>

---

<Primary>

# Why building a component library?

</Primary>

---

## Good Component libraries

- ensure UI consistency
- enable faster development
- abstract implementation details

---

## ~~Good~~ _Great_ Component libraries

- ensure UI consistency
- enable faster development
- abstract implementation details
- _designed for change_
- _intuitive to use_
- _inclusive_
- _based on strong principles_

---

## Ensure UI consistency

---

<Figure
	src="/assets/too-many-buttons.jpg"
	alt="Different button styles"
	caption="Make it impossible to end up here"
/>

---

## Abstract implementation details

---

<Figure
	src={'/assets/bootstrap-dropdown.png'}
	alt="Bootstrap Documentation Screenshot"
	caption={
		<a href="https://getbootstrap.com/docs/3.3/components/#dropdowns">
			https://getbootstrap.com/docs/3.3/components/#dropdowns
		</a>
	}
/>

<ThatGuy />

---

```html
<div class="dropdown">
	<button
		class="btn btn-default dropdown-toggle"
		type="button"
		id="dropdownMenu1"
		data-toggle="dropdown"
		aria-haspopup="true"
		aria-expanded="true"
	>
		Dropdown
		<span class="caret"></span>
	</button>
	<ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
		<li><a href="#">Action</a></li>
		<li><a href="#">Another action</a></li>
		<li><a href="#">Something else here</a></li>
		<li role="separator" class="divider"></li>
		<li><a href="#">Separated link</a></li>
	</ul>
</div>
```

---

```jsx
<DropdownButton title="Title">
	<MenuItem eventKey="1">Action</MenuItem>
	<MenuItem eventKey="2">Another action</MenuItem>
	<MenuItem eventKey="3" active>
		Active Item
	</MenuItem>
	<MenuItem divider />
	<MenuItem eventKey="4">Separated link</MenuItem>
</DropdownButton>
```

---

## Intuitive

- `<input type="button">` → `<Button>`
- `<input type="text">` → `<TextField>`
- `<input type="checkbox">` → `<Checkbox>`

---

## Designed for change

---

```jsx
const Dropdown = (props) => (
	<select>
		{props.options.map((option) => (
			<option>{option}</option>
		))}
	</select>
);
```

---

```jsx
const Dropdown = (props) => (
	<div className="dropdown">
		<button className="btn btn-dropdown">{props.button}</button>
		<ul>
			{props.options.map((option) => (
				<li>{option}</li>
			))}
		</ul>
	</div>
);
```

---

## Inclusive

---

<Grid gap={5} columns={2}>
	<span>A button</span>
	<Code language="jsx">{`<Button>`}</Code>
	<span>A primary button</span>
	<Code language="jsx">{`<Button primary>`}</Code>
	<span>A checked checkbox</span>
	<Code language="jsx">{`<Checkbox checked>`}</Code>
	<span>A product card</span>
	<Code language="jsx">{`<ProductCard>`}</Code>
</Grid>

---

# Examples

---

<Figure
	src={'/assets/mui.png'}
	alt="Material UI Homepage"
	caption={<a href="https://material-ui.com">https://material-ui.com</a>}
	gap={4}
/>

---

<Figure
	src={'/assets/antdesign.png'}
	alt="Ant.Design Homepage"
	caption={<a href="https://ant.design">https://ant.design</a>}
	gap={4}
/>

---

<Figure
	src={'/assets/chakra.png'}
	alt="Chakra UI Homepage"
	caption={<a href="https://chakra-ui.com">https://chakra-ui.com</a>}
/>

---

## Why own component library?

1. 👍 Control
1. 👍 Development Speed
1. 👍 Custom Requirements
1. 👍👎 Maintenance

<Notes>
	- Open source software is often written in a free time by a small amount of
	people. If something goes wrong, you might end up with an unmaintained
	software. I would not bet my business on that. - Ready made component
	libraries are often try to be universal and please everyone. That comes with
	the cost of verbosity, greater choice and complexity. At the same time they
	often don't cover specific requirements of your business. - It's harder to fix
	things in the code that you don't own. If a requirement for one of the
	component changes, it might take weeks to turn over the formal process and get
	a new version even with a well maintained component library. All this time
	you'll probably be maintaining your own fork of the repo with all its bells
	and whistles.
</Notes>

---

<Grid gap={5}>
<Grid columns={2}>

## OSS

ex. Material UI, Ant Design, Bootstrap

</Grid>
<Grid columns={2}>

## Mix

Re-export or wrap components from OSS (Reakit, Theme-UI, Chakra, etc.)

</Grid>
<Grid columns={2}>

## Custom

Build everything from scratch (using styled-system, etc.)

</Grid>
</Grid>

---

<SplitTitle>

## Using open source

```diff
import { DatePicker } from "material-ui"

<DatePicker {...} />
```

</SplitTitle>

---

<SplitTitle>

## Using mixed approach

```jsx
// components/index.js

export { DatePicker } from "material-ui"

// app.js
import { DatePicker } from "./components"

<DatePicker {...} />
```

</SplitTitle>

---

## 🥁🥁🥁 Today we're building

---

<Figure
	src={'/assets/window-prompt.png'}
	alt="window.prompt"
	caption="window.prompt"
/>

---

# 👩‍💻 `npm start`

- [http://localhost:8000](http://localhost:8000) — App
- [http://localhost:6061](http://localhost:6061) — Exercises

---

<Figure
	src={'/assets/react-styleguidist.svg'}
	alt="React-styleguidist Logo"
	caption={<a href="https://react-styleguidist.js.org">React Styleguidist</a>}
	gap={4}
/>

---

<Demo src="//localhost:6061" prodSrc="about:blank" />

---

> 20 minutes

---

<Secondary>

## Recap

</Secondary>

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
	showHeader={false}
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
	showHeader={false}
	submitLabel="Submit"
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
	showHeader={false}
	submitLabel="Submit"
	cancelLabel="Go away!"
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
	showHeader={false}
	submitLabel="Submit"
	cancelLabel="Go away!"
	className="style"
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
	showHeader={false}
	submitLabel="Submit"
	cancelLabel="Go away!"
	className="style"
	closeStyles={{}}
/>
```

---

```jsx
<Prompt
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
	showHeader={false}
	submitLabel="Submit"
	cancelLabel="Go away!"
	className="style"
	closeStyles={{}}
/>
```

---

# 😭

---

> Apropcalipse

---

## Lessons learned

- Wrong abstractions are worse than no abstractions
- Big monolithic components are bad

---

<Primary imgSrc="/assets/legos.jpg">

# UI Primitives

</Primary>

---

> Primitives are the building blocks of the user interface.

---

<Figure
	src={'/assets/legos-assembled.jpg'}
	alt="Lego cars"
	caption="Different items from the same set of bricks"
/>

---

<Primary>

# Compound Components

</Primary>

---

```html
<select name="select">
	<option value="1">compound components</option>
	<option value="2">apropcalipse</option>
</select>
```

---

```html
<table>
	<thead>
		<tr>
			<th>Heading</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>Cell</td>
		</tr>
	</tbody>
</table>
```

---

```html
<table>
	<thead>
		<tr>
			<th>Heading</th>
			<th>Heading</th>
			<th>Heading</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>Cell</td>
			<td>Cell</td>
			<td>Cell</td>
		</tr>
	</tbody>
</table>
```

---

```html
<table>
	<tbody>
		<tr>
			<td>Cell</td>
			<td>Cell</td>
			<td>Cell</td>
		</tr>
	</tbody>
</table>
```

---

> Inversion of control

---

# 👩‍💻 `npm start`

- [http://localhost:8000](http://localhost:8000) — App
- [http://localhost:6061](http://localhost:6061) — Exercises

---

> 20 minutes

---

<Secondary>

## Recap

</Secondary>

---

```jsx
<Prompt
	title="Asking for a name"
	message="What’s your name, yo?"
	defaultValue="Incognito"
	onSubmit={}
	onClose={}
	showClose
	showCancel
	showBackdrop
	showHeader
	submitLabel="Submit"
	cancelLabel="Go away!"
	className="style"
	closeStyles={{}}
/>
```

---

```jsx
<Dialog>
	<DialogBackdrop onClose={} />
	<DialogContent>
		<DialogCloseButton onClose={} />
		<form onSubmit={}>
			<DialogHeader>The universe asks</DialogHeader>
			<DialogBody>
				<label>
					What’s your name, yo?
					<input type="text" />
				</label>
			</DialogBody>
			<DialogFooter>
				<button type="button" onClick={onClose}>
					Cancel
				</button>
				<button type="submit">OK</button>
			</DialogFooter>
		</form>
	</DialogContent>
</Dialog>
```

---

## hardcode → flexibility

---

## Lessons learned

- Inversion of control
- Create UI primitives
- Compose complex UI from them

---

<Primary>

# Principles of Primitives

</Primary>

---

- Pass down valid HTML attributes
- Always pass the `ref` with `forwardRef`
- Allow setting underlying HTML tag with `as`
- Merge styles and `className`
- Merge event handlers
- No outer white space (`margin: 0`)

---

<SplitTitle>

## Always pass the `ref` with `forwardRef`

```jsx
const Input = ({ type, value, onChange }) => (
	<input type={type} value={value} onChange={onChange} />
);

// In your app

const inputRef = React.useRef(null);

inputRef.current.focus() // Not going to work!

<>
	<Input ref={inputRef}>
</>
```

</SplitTitle>

---

<SplitTitle>

## Always pass the `ref` with `forwardRef`

```diff
- const Input = ({ type, value, onChange }, ref) => (
+ const Input = React.forwardRef(({ type, value, onChange }, ref)) => (
	<input
+		ref={ref}
		type={type}
		value={value}
		onChange={onChange}
	/>
-);
+));
```

</SplitTitle>

---

<SplitTitle>

## Pass down valid HTML attributes

```jsx
const Input = ({ type, value, onChange }) => (
	<input type={type} value={value} onChange={onChange} />
);

// In your app

<Input placeholder="Not going" aria-label="to work"> // Ooops!
```

</SplitTitle>

---

<SplitTitle>

## Pass down valid HTML attributes

```diff
- const Input = ({ type, value, onChange }) => (
+ const Input = ({ type, value, onChange, ...rest }) => (
-	<input type={type} value={value} onChange={onChange} />
+	<input type={type} value={value} onChange={onChange} {...rest} />
);
```

</SplitTitle>

---

<SplitTitle>

## Allow setting underlying HTML tag with `as`

```jsx
const Button = (props) => <button {...props} />;

// In your app

<Button as="a" href="#">
	{' '}
	{/* Ooops! */}
	Link button
</Button>;
```

</SplitTitle>

---

<SplitTitle>

## Allow setting underlying HTML tag with `as`

```jsx
const Button = ({ as: Component = 'button', ...props }) => (
	<Component {...props} />
);
```

</SplitTitle>

---

<SplitTitle>

## Merge styles and `className`

```jsx
const RedBox = (props) => (
	<div className="p-3 bg-red" {...props} />
)

// In you app

<RedBox className="customize"/> // Oops!
```

</SplitTitle>

---

<SplitTitle>

## Merge styles and `className`

```jsx
import cx from "classnames";

const RedBox = ({className, ...props}) => (
	<div className={cx("p-3 bg-red", className)} {...props} />
)

// In you app

<RedBox className="customize"/> // All 3 class names applied
```

</SplitTitle>

---

## Merge event handlers

---

## No outer white space (`margin: 0`)

---

> Primitives should not assume how they will be used

---

<Primary>

# State encapsulation

</Primary>

---

```jsx
const [isExpanded, setIsExpanded] = React.useState(false);
<>
	<DisclosureButton onClick={() => setIsExpanded((x) => !x)}>
		Toggle
	</DisclosureButton>
	{isExpanded && <DisclosureContent>Content</DisclosureContent>}
</>;
```

---

```jsx
function useDisclosureState({ expanded = false } = {}) {
	const [isExpanded, setIsExpanded] = React.useState(expanded);
	return {
		isExpanded,
		onToggle: () => setIsExpanded((x) => !x),
	};
}

const disclosure = useDisclosureState({ expanded: false });
<>
	<DisclosureButton {...disclosure}>Toggle</DisclosureButton>
	<DisclosureContent {...disclosure}>Content</DisclosureContent>
</>;
```

---

# 👩‍💻 `npm start`

- [http://localhost:8000](http://localhost:8000) — App
- [http://localhost:6061](http://localhost:6061) — Exercises

---

> 15-20 minutes

---

<Secondary>

# Recap

</Secondary>

---

## Lessons learned

- Hooks allow encapsulation and sharing of complex state
- Provide a way to override or access state

---

<Primary>

# Headless Components

</Primary>

---

> If a component doesn't render HTML it can be a hook

---

- react-dropzone
- downshift
- Reakit hooks

---

```jsx
import React, { useCallback } from 'react';
import { useDropzone } from 'react-dropzone';

function MyDropzone() {
	const onDrop = React.useCallback((acceptedFiles) => {
		// Do something with the files
	}, []);
	const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop });

	return (
		<div {...getRootProps()}>
			<input {...getInputProps()} />
			{isDragActive ? (
				<p>Drop the files here ...</p>
			) : (
				<p>Drag'n'drop some files here, or click to select files</p>
			)}
		</div>
	);
}
```

---

> Low-level primitives to create UI Patterns

---

<Primary>

# UI Patterns

</Primary>

---

> A UI pattern is the re-usable form of a solution to a design problem.

---

```jsx
<Dialog>
	<DialogContent>
		<form onSubmit={}>
			<DialogHeader>Asking for a name</DialogHeader>
			<DialogBody>
				<label>
					What’s your name, yo?
					<input type="text" defaultValue="Incognito" />
				</label>
			</DialogBody>
			<DialogFooter>
				<button type="button" onClick={onClose}>
					Cancel
				</button>
				<button type="submit">OK</button>
			</DialogFooter>
		</form>
	</DialogContent>
</Dialog>
```

---

```jsx
<Dialog>
	<DialogContent>
		<form onSubmit={}>
			<DialogHeader>What’s 2 + 2?</DialogHeader>
			<DialogBody>
				<label>
					Please enter the answer:
					<input type="number" />
				</label>
			</DialogBody>
			<DialogFooter>
				<button type="button" onClick={onClose}>
					Cancel
				</button>
				<button type="submit">OK</button>
			</DialogFooter>
		</form>
	</DialogContent>
</Dialog>
```

---

# UI Patterns

- UI with a specific goal (solution)
- Better developer experience (API is more specific)
- Built with primitives

---

# 👩‍💻 `npm start`

- [http://localhost:8000](http://localhost:8000) — App
- [http://localhost:6061](http://localhost:6061) — Exercises

---

<Secondary>

# Recap

</Secondary>

---

# Primitives

- 👍 Less abstractions
- 👍 Easy to change
- 👎 Inconsistent UI
- 👎 Slower development

---

# Patterns

- 👎 Higher-order abstractions
- 👎 Harder to change
- 👍 Consistent UI
- 👍 Faster development

---

## flexibility → rigidity

---

<TweetEmbed id="1090733766950223878" />

---

## Compound components in Patterns?

---

> If position matters or different children can be used, use compound components. Otherwise use props.

---

## Compound Components

<Code language="jsx">
	{`<Menu>
	<MenuItem>Item 1</MenuItem>
	<MenuItem>Item 2</MenuItem>
	<MenuItemDivider/>
	<MenuItem>Item 4</MenuItem>
</Menu>`}
</Code>

---

## Prop-based API

<Code language="jsx">
	{`<Prompt message="Don't want to know about the layout" onSubmit={} />`}
</Code>

---

## Combine both approaches

<Code language="jsx">
	{`<DropdownMenu trigger={<Button>Open menu</Button>}>
	<MenuItem>Item 1</MenuItem>
	<MenuItem>Item 2</MenuItem>
	<MenuItemDivider/>
	<MenuItem>Item 4</MenuItem>
</DropdownMenu>`}
</Code>

---

<Primary>

# React Context

</Primary>

---

> Context is designed to share data that can be considered “global” for a tree of React components

---

<SplitTitle>

## Avoid "prop drilling"

```jsx
import { ThemeProvider } from 'styled-components';
import theme from '../theme';

<ThemeProvider theme={theme}>
	<App />
</ThemeProvider>;
```

</SplitTitle>

---

## Avoid re-renders by bypassing React reconciliation

[React at 60fps](https://medium.com/hackernoon/react-at-60fps-4e36b8189a4c)

---

<Demo src="https://react-sync-scroll.netlify.app/" />

---

## Manage global state, decouple rendering, provide better APIs

---

# 👩‍💻 `npm start`

- [http://localhost:8000](http://localhost:8000) — App
- [http://localhost:6061](http://localhost:6061) — Exercises

---

> 15-20 minutes

---

<Secondary>

# Recap

</Secondary>

---

## React Context

- global state store (you may not need Redux!)
- performance optimization
- decouple rendering from intent
- remove boilerplate
- simplify APIs
- something else?..

---

<Primary>

# Q & A

</Primary>

---

<Secondary>

## [Feedback Form](https://forms.gle/KxAzmBVCbE5JP3cZA)

</Secondary>

---

<Figure
	src={'/assets/component-driven.svg'}
	alt="Component-driven Logo"
	caption={
		<a href="https://component-driven.dev">https://component-driven.dev</a>
	}
/>

---

<Primary>

# Thank you!

<Grid gridGap={[3, 5]} columns={[1, 3]} pt={4}>
	<a href="https://twitter.com/okonetchnikov">@okonetchnikov</a>
	<a href="https://twitter.com/iamsapegin">@iamsapegin</a>
	<a href="https://twitter.com/ComponentDriven">@ComponentDriven</a>
</Grid>

</Primary>
